...
// uint8_t OLED_GRAM[SCREEN_WIDTH_SIZE][SCREEN_HEIGHT_SIZE];
uint8_t OLED_GRAM[SCREEN_HEIGHT_SIZE][SCREEN_WIDTH_SIZE];
...
{
	uint8_t y,x;
	for(y=0;y<SCREEN_HEIGHT_SIZE;y++)
	{
		OLED_WR_Byte(0xb0+y,OLED_CMD); //设置行起始地址
		OLED_WR_Byte(0x00,OLED_CMD);   //设置低列起始地址
		OLED_WR_Byte(0x10,OLED_CMD);   //设置高列起始地址
		SCREEN_I2C_WRITE(0x78,0x40,OLED_GRAM[y],SCREEN_WIDTH_SIZE);
  }
}
//清屏函数
void OLED_Clear(void)
{
	uint32_t len = SCREEN_WIDTH_SIZE * SCREEN_HEIGHT_SIZE;
	uint8_t *p = (uint8_t *)OLED_GRAM;
	while (--len)
	{
		*p = 0;
		p++;
